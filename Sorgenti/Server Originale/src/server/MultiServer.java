 ...
 * 						  � attiva si utilizza il Socket ottenuto in un nuovo Thread per servire un particolare Client. Il thread principale, 
 * 						  intanto, richiamer� accept() per attendere un nuovo client. </p>
		...
		try {
			...
			try {
				while(true) {
					...
					try {
						System.out.println("Connection accepted on " + socket.getInetAddress() + " on Socket Port " + socket.getPort());
						
						new ServerOneClient(socket);
						
					} catch (IOException e) {	
						System.out.println("Accept failed on " + socket.getInetAddress() + " on Socket Port " + socket.getPort());
						socket.close();
